      ...
      <div style={{ background: 'white', borderRadius: '16px', border: '1px solid #e2e8f0', overflow: 'hidden', boxShadow: '0 1px 3px rgba(0,0,0,0.04)' }}>
        ...
        <div style={{ padding: '1.5rem' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
            ...
            <div>
              <label style={s.label}>
                <User size={13} color="#2563eb" /> Nombre completo
              </label>
              {editando ? (
                <input
                  value={form.nombre}
            ...
            <div>
              <label style={s.label}>
                <Mail size={13} color="#2563eb" /> Correo electronico
              </label>
            ...
            <div>
              <label style={s.label}>
                <Shield size={13} color="#2563eb" /> Rol en la plataforma
              </label>
          ...
          <div style={{ display: 'flex', gap: '0.75rem', marginTop: '1.5rem' }}>
            {!editando ? (
              <button onClick={() => setEditando(true)} style={{ display: 'flex', alignItems: 'center', gap: '0.4rem', padding: '0.65rem 1.25rem', background: '#2563eb', color: 'white', border: 'none', borderRadius: '10px', fontWeight: '600', fontSize: '0.875rem', cursor: 'pointer' }}>
                <Edit2 size={14} /> Editar perfil
              </button>
            ) : (
              <>
                <button onClick={handleGuardar} style={{ display: 'flex', alignItems: 'center', gap: '0.4rem', padding: '0.65rem 1.25rem', background: '#2563eb', color: 'white', border: 'none', borderRadius: '10px', fontWeight: '600', fontSize: '0.875rem', cursor: 'pointer' }}>
                  <Save size={14} /> Guardar cambios
                </button>
                <button onClick={() => { setEditando(false); setForm({ nombre: user?.nombre || '', email: user?.email || '' }); }} style={{ display: 'flex', alignItems: 'center', gap: '0.4rem', padding: '0.65rem 1rem', background: 'white', color: '#64748b', border: '1px solid #e2e8f0', borderRadius: '10px', fontWeight: '600', fontSize: '0.875rem', cursor: 'pointer' }}>
                  <X size={14} /> Cancelar
                </button>
              </>
            )}
